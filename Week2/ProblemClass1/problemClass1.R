...
MyDates = as.Date(MyData$Date, "%d/%m/%Y") plot(MyDate, MyData$GBPtoEUR, type = 'l')

# Now we can compare the initial plot with the modified plot

par(mfrow=c(1, 2)) plot(MyData$GBPtoEUR) plot(MyDates, MyData$GBPtoEUR, type="l",col="blue", main="EUR/GBP exchange rate", xlab="Time", ylab="GBP per one EUR", cex.main =1.25, cex.lab=1.5, cex.axis=2) par(mfrow=c(1,1))
